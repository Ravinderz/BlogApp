...
config(['$routeProvider',function($routeProvider){
	$routeProvider.when('/readpost',{
	 ...
	 if(sessionStorage.getItem("user")){
		...
		$http({
			method: "POST",
			url : "http://localhost:80/api/v1.0/post/findPostById/"+postId,
			header:{
		...
		$http({
			method: "POST",
			url : "http://localhost:80/api/v1.0/post/likePost/"+id,
			header:{
				'Content-Type':'application/json'
			}
		}).then(function(response){
		...
		$http({
			method: "POST",
			url : "http://localhost:80/api/v1.0/post/likeComment/"+id,
			header:{
				'Content-Type':'application/json'
			}
		}).then(function(response){
		...
		$http({
			method: "POST",
			url : "http://localhost:80/api/v1.0/post/addComment/"+postId,
